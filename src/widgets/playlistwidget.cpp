...
void PlaylistWidget::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if(item != nullptr)
    {
        ...
        // --- ДОБАВЛЯЕМ МЕНЮ ВЫБОРА КАЧЕСТВА ---
        QMenu *qualityMenu = menu->addMenu(tr("Качество (AVC)"));
        QList<int> resolutions = {144, 240, 360, 480, 720, 1080, 1440, 2160};

        for(int res : resolutions)
        {
            // Создаем строку, например "720p"
            QString label = QString("%0p").arg(res);
            
            // При клике вызываем нашу функцию в mpvhandler
            connect(qualityMenu->addAction(label), &QAction::triggered, [this, res]()
            {
                if(baka && baka->mpv)
                    baka->mpv->SetQuality(res);
            });
        }
        menu->addSeparator(); // Черта для отделения от стандартных команд
        // ---------------------------------------

        connect(menu->addAction(tr("R&emove from Playlist")), &QAction::triggered,
                [=]
                {
                    RemoveFromPlaylist(item);
                });
        connect(menu->addAction(tr("&Delete from Disk")), &QAction::triggered,
                [=]
                {
                    DeleteFromDisk(item);
                });
        connect(menu->addAction(tr("&Refresh")), &QAction::triggered,
                [=]
                {
                    RefreshPlaylist();
                });
