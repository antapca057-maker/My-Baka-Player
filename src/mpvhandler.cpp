...
} // <-- Вот здесь HandleErrorCode ЗАКОНЧИЛАСЬ

void MpvHandler::SetQuality(int height)
{
    if(mpv)
    {
        // Настраиваем yt-dlp: ищем AVC (h264) не выше выбранной высоты
        // [vcodec^=avc1] — это как раз фильтр для AVC формата
        QString formatString = QString("bestvideo[height<=%1][vcodec^=avc1]+bestaudio/best[height<=%1]").arg(height);
        
        QByteArray tmp = formatString.toUtf8();
        HandleErrorCode(mpv_set_option_string(mpv, "ytdl-format", tmp.constData()));
        
        ShowText(tr("Качество: %0p (AVC)").arg(height), 3000);

        // Если уже что-то играет из сети — перезагружаем в новом качестве
        if(playState > Mpv::Idle && Util::IsValidUrl(file))
        {
            OpenFile(file); 
        }
    }
